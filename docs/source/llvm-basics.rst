...
#. 汇编器做了什么？
#. 链接器做了什么？
#. 通过编写 LLVM IR 程序，熟悉 LLVM IR 中间语言。

并尽可能地对其实现方式有所了解。

方法
^^^^^^^
以一个简单的 C（C++）源程序为例（你也可以用 Rust），调整编译器的程序选项获得 **各阶段的输出** ，研究它们与源程序的关系，以此撰写调研报告。二进制文件或许需要利用某些系统工具理解，如 ``objdump`` 、 ``nm``。
...
不要当作“命题作文”，要更多地发挥主观能动性，将其当做实验进行更多探索。如：
