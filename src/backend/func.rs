...
impl MFuncData {
    ...
    pub fn self_ptr(&self) -> MFunc { self.self_ptr }
}
...
impl MFunc {
    ...
    pub fn label(self, arena: &MContext) -> &MLabel { &self.deref(arena).label }
    ...
    pub fn storage_stack_size(self, mctx: &MContext) -> u64 { self.deref(mctx).storage_stack_size }
...
impl Arena<MFunc> for MContext {
    ...
    fn try_deref(&self, ptr: MFunc) -> Option<&MFuncData> { self.funcs.try_deref(ptr.0) }
...
impl LinkedListContainer<MBlock> for MFunc {
    ...
    fn head(self, ctx: &Self::Ctx) -> Option<MBlock> { self.deref(ctx).head }

    fn tail(self, ctx: &Self::Ctx) -> Option<MBlock> { self.deref(ctx).tail }
